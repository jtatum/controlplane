...
beforeEach(async () => {
  ...
  await pool.query(
    "UPDATE users SET role = 'admin' WHERE external_id = 'dev-user-001'",
  );
});
...
async function seedEmailChannel(
  agentId: string,
  opts: { outboundReview?: boolean } = {},
) {
  ...
  await pool.query(
    ...
    [
      channelId,
      `${agentId.slice(0, 8)}@openclaw.test`,
      opts.outboundReview ?? true,
    ],
  );
  return channelId;
}

async function seedEmailMessage(
  agentId: string,
...
describe("POST /api/agents", () => {
  ...
  it("returns 409 when non-admin user already has an active agent", async () => {
    await pool.query(
      "UPDATE users SET role = 'user' WHERE external_id = 'dev-user-001'",
    );
  ...
  it("allows non-admin to create again after terminating previous agent", async () => {
    ...
    const first = await freshApp.inject({
      ...
      payload: {
        name: "First Agent",
        agentName: "term-first",
        environment: "dev",
      },
    });
    expect(first.statusCode).toBe(201);

    await pool.query("UPDATE agents SET status = 'terminated' WHERE id = $1", [
      first.json().id,
    ]);
...
describe("GET /api/agents", () => {
  ...
  it("supports pagination with limit and offset", async () => {
    ...
    const res = await app.inject({
      method: "GET",
      url: "/api/agents?limit=1&offset=1",
    });
    const body = res.json();
  ...
  it("filters by environment", async () => {
    ...
    const res = await app.inject({
      method: "GET",
      url: "/api/agents?environment=prod",
    });
    const body = res.json();
    expect(body.data).toHaveLength(1);
...
describe("POST /api/agents/:id/terminate", () => {
  it("initiates termination and returns stopping status", async () => {
    ...
    const res = await app.inject({
      method: "POST",
      url: `/api/agents/${agent.id}/terminate`,
    });
    expect(res.statusCode).toBe(200);
  ...
  it("returns 404 for nonexistent agent", async () => {
    const res = await app.inject({
      method: "POST",
      url: "/api/agents/00000000-0000-0000-0000-000000000000/terminate",
    });
    expect(res.statusCode).toBe(404);
  });
  ...
  it("returns 409 for already stopping agent", async () => {
    ...
    await pool.query("UPDATE agents SET status = 'stopping' WHERE id = $1", [
      agent.id,
    ]);
    const res = await app.inject({
      method: "POST",
  ...
  it("returns 409 for agent in requested status", async () => {
    ...
    await pool.query("UPDATE agents SET status = 'requested' WHERE id = $1", [
      agent.id,
    ]);
    const res = await app.inject({
      method: "POST",
...
describe("GET /api/emails/review", () => {
  ...
  it("filters by status", async () => {
    ...
    const pending = await app.inject({
      method: "GET",
      url: "/api/emails/review?status=pending",
    });
    expect(pending.json().messages).toHaveLength(1);

    const approved = await app.inject({
      method: "GET",
      url: "/api/emails/review?status=approved",
    });
  ...
  it("returns 400 for invalid status filter", async () => {
    const res = await app.inject({
      method: "GET",
      url: "/api/emails/review?status=invalid",
    });
    expect(res.statusCode).toBe(400);
  ...
  it("supports pagination", async () => {
    ...
    const page1 = await app.inject({
      method: "GET",
      url: "/api/emails/review?limit=2&offset=0",
    });
    ...
    const page2 = await app.inject({
      method: "GET",
      url: "/api/emails/review?limit=2&offset=2",
    });
  ...
  it("filters by agentId", async () => {
    ...
    const res = await app.inject({
      method: "GET",
      url: `/api/emails/review?agentId=${agent1.id}`,
    });
...
describe("GET /api/emails/:messageId", () => {
  ...
  it("returns 404 for nonexistent message", async () => {
    const res = await app.inject({
      method: "GET",
      url: "/api/emails/00000000-0000-0000-0000-000000000000",
    });
    expect(res.statusCode).toBe(404);
...
describe("POST /api/emails/:messageId/review", () => {
  it("approves a pending message", async () => {
    ...
    const dbCheck = await pool.query(
      "SELECT visible_to_agent FROM email_messages WHERE id = $1",
      [messageId],
    );
...
describe("agent ownership checks", () => {
  ...
  describe("admin bypass", () => {
    it("admin can list all agents including other users'", async () => {
      await createAgentForUser(otherUserId, "other-list");
      await createAgent("own-list-agent");
    ...
    it("admin can view agent detail for another user's agent", async () => {
      const agentId = await createAgentForUser(otherUserId, "other-detail");
      const res = await app.inject({
        method: "GET",
  ...
  describe("non-admin ownership denial", () => {
    it("GET /agents only returns own agents for non-admin", async () => {
      await createAgentForUser(otherUserId, "other-hidden");
      await createAgent("own-visible");
      await pool.query(
        "UPDATE users SET role = 'user' WHERE external_id = 'dev-user-001'",
      );

      const freshApp = await createFreshApp();
      const res = await freshApp.inject({ method: "GET", url: "/api/agents" });
      expect(res.statusCode).toBe(200);
      expect(res.json().data).toHaveLength(1);
      expect(res.json().data[0].agentName).toBe("own-visible");
      expect(res.json().total).toBe(1);
      await freshApp.close();
    });

    it("GET /agents/:id returns 404 for non-owner", async () => {
      const agentId = await createAgentForUser(
        otherUserId,
        "other-detail-deny",
      );
      await pool.query(
  ...
  describe("email review cross-user isolation", () => {
    ...
    it("admin can see all emails in review queue", async () => {
      const otherAgentId = await createAgentForUser(
        otherUserId,
        "admin-review",
      );
...
describe("GET /api/agents/:agentId/emails/inbox", () => {
  ...
  it("returns 404 for nonexistent agent", async () => {
    const res = await app.inject({
      method: "GET",
      url: "/api/agents/00000000-0000-0000-0000-000000000000/emails/inbox",
    });
    expect(res.statusCode).toBe(404);
  });
});
...
async function setAgentToken(agentId: string, rawToken: string) {
  ...
  await pool.query("UPDATE agents SET agent_token_hash = $1 WHERE id = $2", [
    hash,
    agentId,
  ]);
  return hash;
